...
import com.jovellanos.clicker.audio.AudioManager.Track;
...
public class MainMenuScreen extends BaseScreen {
    ...
    @Override
    public void show() {
        AudioManager.getInstance().playMusic(AudioManager.Track.MENU);
        super.show();
    }
        ...
        for (Actor actor : stage.getActors()) {
            if (actor instanceof Dialog) {
                actor.setPosition(Math.round((stage.getWidth() - actor.getWidth()) / 2),
                        Math.round((stage.getHeight() - actor.getHeight()) / 2));
            }
        }
    }
